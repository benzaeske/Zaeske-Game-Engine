...
class SpatialPartitioningModel:
    ...
    def process_player_fish_coherency(self, dt: float) -> None:
        r: int = int(self.player.position.y / self.world_specifications.cell_size)
        c: int = int(self.player.position.x / self.world_specifications.cell_size)
        cohere_red: int = 0
        cohere_green: int = 0
        cohere_yellow: int = 0
    ...
    def find_neighbors_and_make_schooling_decisions(self, current_fish: Fish) -> None:
        ...
        c: int = int(current_fish.position.x / self.world_specifications.cell_size)
        for dr in range(-cell_range, cell_range + 1):
            for dc in range(-cell_range, cell_range + 1):
                grid_r: int = r + dr
                grid_r = (
    ...
    def update_jellyfish(self, dt: float):
        ...
        for jellyfish in self.jellyfish.values():
            ...
            c: int = int(jellyfish.position.x / self.world_specifications.cell_size)
