...
from model.entities.entity import FrameActionContext
from model.entitygroups.entitygroup import EntityGroup


class EnemyGroup(EntityGroup):
    """
    Base class for a group of enemies that continue to spawn in on a given cooldown interval.
    """
    ...
    def spawn(self, context: FrameActionContext) -> None:
        ...
        for _ in range(self._amount):
            new_enemy: Enemy = self.get_new_enemy()
            new_enemy.set_position(self._get_initial_position(context))
            context.grid_space_access.add_entity(new_enemy, None)
    ...
    def _get_initial_position(context: FrameActionContext) -> Vector2:
        ...
        """
        world_specs = context.world_specs
